...
export class FeedbackController {
    ...
    @Roles('admin')
    @UseGuards(AuthGuard)
    async createFeedback(@Param('candidateId') candidateId: string,
        @GetUser() user: UserRecord,
        @Body() feedbackBody: Feedback) {
        const data = await this.feedbackService.create(candidateId, user.uid, feedbackBody)
        return data
    }

    @Patch(':id')
    async patchFeedback(@Param('id') id: string,
    ...
    async deleteFeedback(@Param(':id') id: string) {
        const data = await this.feedbackService.delete(
            id,
        )
        return data
    }
